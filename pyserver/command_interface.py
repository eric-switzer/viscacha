...
class CommandInterface(CommandHandler):
    ...
    def sendcmd_all(self, line):
        for session in self.sessions:
            if self.form_command(line) != None:
                commentpush(session, 'SERVER_CMDOUT',
                            self.form_command(line))
    ...
    def do_togglelock(self, session, line):
        if self.lockcontrols:
            ...
            self.broadcast(SERVER_TOKEN + session.name + \
                           ' UNLOCKED session controls')
        else:
            self.lockcontrols = 1
            self.broadcast(SERVER_TOKEN + session.name + \
                           ' LOCKED session controls')
    ...
    def do_controldbput(self, session, line):
        ...
        if refresh_type == 'value' or refresh_type == 'reply':
            if short_name in self.server.db.keys():
                ...
                variable_type = control_parse.find_controltype(rec['type'],
                                                               MASTER_SHELVE)

                rec['default_val'] = \
                    control_parse.control_typecast(refresh_val, variable_type)

                self.server.db[short_name] = rec

                silentlogpush('SERVER_MESSAGE',
                              'ClientInterface: changed ' + \
                              short_name + ' to ' + refresh_val)

                self.do_human_put(session, 'gui_message ' + short_name + ' ' \
                                           + refresh_type + ' ' + refresh_val)
            else:
                silentlogpush('SERVER_MESSAGE',
                              'command_interface: bad db short name')
    ...
    def do_refresh(self, session, line):
        for key in self.server.db.keys():
            try:
                ...
                silentlogpush('SERVER_MESSAGE',
                              'control entry missing "short_name" entry')
        ...
        if len(cmdstring) == 3:
            ...
            if not self.lockcontrols:
                if user in self.server.users:
                    logline = 'uput request to user=%s, \
                               short_name=%s, command_value=%s' % \
                              (user, short_name, command_value)
                    ...
                    if silent == True:
                        ...
                        varchange = repr(control_parse.get_db_convert(
                                         MASTER_SHELVE, 'short_name',
                                         short_name, 'desc'))
                    ...
                    write_to_user(self.server, user,
                                  short_name + ' ' + command_value)
                else:
                    commentpush(session, 'SERVER_MESSAGE',
                                SERVER_TOKEN + user + \
                                ' is not connected to the interface server')
            else:
                commentpush(session, 'SERVER_MESSAGE',
                            SERVER_TOKEN + 'SESSION IS LOCKED')
        else:
            commentpush(session, 'SERVER_MESSAGE',
                        SERVER_TOKEN + 'uput: bad number of arguments')
    ...
    def tput(self, session, line, silent):
        ...
        if len(cmdstring) == 3:
            ...
            if not self.lockcontrols:
                logline = 'tput request to interface_type=%s, \
                          short_name=%s, command_value=%s' % \
                          (interface_type, short_name, command_value)
                ...
                if silent == True:
                    ...
                    silentlogpush('SERVER_MESSAGE', SERVER_TOKEN + logline)
                    varchange = repr(control_parse.get_db_convert(
                            MASTER_SHELVE, 'short_name', short_name, 'desc'))

                    self.broadcast(SERVER_TOKEN + session.name + \
                                   ' changed ' + varchange + \
                                   ' to ' + repr(command_value))
                # commit the request to all users of that interface type
                write_to_all(self.server, interface_type,
                             short_name + ' ' + command_value)
            else:
                commentpush(session, 'SERVER_MESSAGE',
                            SERVER_TOKEN + 'SESSION IS LOCKED')
            ...
            commentpush(session, 'SERVER_MESSAGE',
                        SERVER_TOKEN + 'tput: bad number of arguments')
        ...
        if len(cmdstring) != 4:
            ...
            if not self.lockcontrols:
                if check_destination(destination):
                    logline = 'dput request to destination=%s, \
                               short_name=%s, command_value=%s' % \
                               (destination, short_name, command_value)
                    ...
                    if silent == True:
                        ...
                        self.broadcast(SERVER_TOKEN + session.name + \
                                        ...
                                        ' to ' + repr(command_value))
                    # commit the request to all users of that interface type
                    write_to_destination(self.server, destination,
                                         short_name + ' ' + command_value)
                else:
                    commentpush(session, 'SERVER_MESSAGE',
                                SERVER_TOKEN + destination + \
                                ' is not bound to any clients')
            else:
                commentpush(session, 'SERVER_MESSAGE',
                            SERVER_TOKEN + 'SESSION IS LOCKED')
        else:
            commentpush(session, 'SERVER_MESSAGE',
                        SERVER_TOKEN + 'dput: bad number of arguments')

    # LPUT: send a message to a logger
    # if there is no logger client, the message, is ignored.
    ...
    def lput(self, session, line, silent):
        ...
        if len(cmdstring) >= 2:
            ...
            commentpush(session, 'SERVER_MESSAGE',
                        SERVER_TOKEN + 'lput: bad number of arguments')
