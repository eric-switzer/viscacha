import os, sys, string, shelve
import signal, resource
import socket, asyncore, time, threading, asynchat
from param import *
import daemonize, control_parse, command_interface
from logger import *
...
class InterfaceSession(asynchat.async_chat):
  ...
  def found_terminator(self):
    line = ''.join(self.data)
    self.data = []
    try: self.interface.handle(self, line)
    except EndSession:
      self.handle_close()
...
if __name__ == '__main__':
  if DAEMONIZE == 'yes':
    ...
    open("/var/run/interface_server.pid", "w").write(repr(os.getpid()) + "\n")
