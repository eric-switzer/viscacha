...
class AmcpInterface(command_interface.CommandInterface):
    ...
    def form_command(self, line):
        ...
        if len(cmdstring) != 2:
            ...
            cmdnum = int(float(control_parse.get_db_convert(MASTER_SHELVE,
                         'short_name', cmdstring[0], 'hwid')))
        ...
        for key in self.server.db.keys():
            try:
                ...
                if 'amcp' in identification_db[rec['destination']]:
                    cmdnum = int(float(control_parse.db_convert(db,
                                 'short_name', rec['short_name'], 'hwid')))
                    cmdval = float(rec['default_val'])
                    refresh_line = '%5d, %10.10f;' % (cmdnum, cmdval)
                    #refresh_line = rec['short_name'] + ' ' \
                    #                + repr(rec['default_val'])
                    #write_to_destination(self.server,
                    #                     rec['destination'], refresh_line)
