...
class LoginPhase(command_interface.CommandInterface):
    ...
    def unknown(self, session, cmd):
        commentpush(session, 'SERVER_AUTH',
                    SERVER_TOKEN +
                    'login <client_name> <interface_type> <control version number>')

    def do_login(self, session, line):
        chop = line.split()
        if len(chop) == 3:
            # parse the login string into the username,
            # interface type and version control tag
            name = chop[0].strip()
            interface_type = chop[1].strip()
            version_tag = chop[2].strip()

            logline = 'attemped user=%s, interface_type=%s, ver_tag=%s' % \
                      (name, interface_type, version_tag)
            ...
            if name not in self.server.users:
                if version_tag == CONTROL_VERSION:
                    ...
                    try:
                        ...
                        commentpush(session, 'SERVER_AUTH',
                                    SERVER_TOKEN +
                                    'no such interface category')

                        raise EndSession
                else:
                    commentpush(session, 'SERVER_AUTH',
                                SERVER_TOKEN + 'error: bad version number')

                    raise EndSession
            else:
                commentpush(session, 'SERVER_AUTH',
                            SERVER_TOKEN +
                            'error: client %s is already logged in.' % name)

                raise EndSession
        else:
            commentpush(session, 'SERVER_AUTH',
                        SERVER_TOKEN + 'error: bad client name/version tag')
