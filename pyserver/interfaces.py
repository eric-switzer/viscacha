...
def amcp_login (session, server):
  commentpush(session,'SERVER_AUTH',SERVER_TOKEN+'AMCP login successful')
...
def mce_login (session, server):
  commentpush(session,'SERVER_AUTH',SERVER_TOKEN+'MCE login successful')
...
def generic_login (session, server):
  commentpush(session,'SERVER_AUTH',SERVER_TOKEN+'generic login successful')
...
def client_login (session, server):
  commentpush(session,'SERVER_AUTH',SERVER_TOKEN+'human login successful')
...
def write_to_user(server, user, line):
  for interface_key in interface_db.keys():
    ...
    if user in rec['who'](server):
      ...
      silentlogpush('SERVER_MESSAGE', 'requested user='+user+' not found')
