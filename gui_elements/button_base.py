...
class ButtonBase(Pmw.MegaWidget):
    ...
    def __init__(self, 
                 ...
                 parent = None,
                 **kw):
        """ Instantiate buttons with properties, and let them listen to IO queues
        """
        optiondefs = (
        ...
        self.buttonlabelcontainer = self.createcomponent('buttonlabelcontainer',
                (), None,
        ...
        self.currentcontainer = self.createcomponent('currentcontainer',
                (), None,
                Pmw.Group, interior, tag_pyclass = None)
        self.currentcontainerlabel = Tkinter.Label(self.currentcontainer.interior(),
                text = 'cur: ', bg = '#C2AD67')
        self.currentcontainerlabel.pack(padx = 0, pady = 0, side=Tkinter.LEFT, fill=Tkinter.X)
        ...
        if self.buttonrec['edit_level'] == 1:
          ...
          if self.buttonrec['edit_level'] == 0: btext = " < "
          if self.buttonrec['edit_level'] == 2: btext = " "
          self.editmode = self.createcomponent('editmode',
                  (), None,
    ...
    ##
    # if the edit level is not set to display mode, the generate the edit dialogs
    def _geneditdialogs(self):
      ...
      self.selectioncontainerlabel = Tkinter.Label(self.selectioncontainer.interior(),
              text = 'sel: ', bg = '#C2AD67')
      ...
      self.safetycontainer = self.createcomponent('safetycontainer',
              (), None,
              Pmw.Group, self.entrycontainer.interior(), tag_pyclass = None)
      self.safetycontainerlabel = Tkinter.Label(self.safetycontainer.interior(),
              text = 'safe: ')
    ...
    def _thesame(self, valuein, testvalue):
      """method to determine whether the echoed command change from the server is 
         "the same" as the requested change.  This should be overridden in subclasses
         to reflect what 'the same' means."""
      try:
    ...
    def refresh_button(self,type,value):
      ...
      if type == 'lock':
        ...
        self._setlock()
      if type == 'value':
        print self.announce+'refreshing buttooooon value to '+value
      ...
      if type == 'reply':
        ...
        if self._thesame(value,self.lastrequest):
          ...
          print self.announce+'reply from server ('+value+') ' + \
                'does not match request ('+self.lastrequest+'), locking button'
          self.lockstate = 1 
          self._setlock()
    ...
    def _currentdisplaymethod(self):
      if self.buttonrec.has_key('current_display_width'):
    ...
    def _issueCommand(self):
      """after issuing the command, set the lock flag and wait for a server response
         if the server responds with a new value, and that value is the same as the one
         requested, then lock->unlocked, indicating that the command was committed
      """
      fieldvalue = control_typecast(self._entryaccess(),'string')
      self.confirmbutton = confirm_button.ConfirmButton(self.buttonrec,fieldvalue)
      if self.confirmbutton.confirmed:
        print self.announce+'Command confirmed'
        print self.announce+'sending ' + fieldvalue
        self.commandqueue.put('dput_verbose '+self.buttonrec['destination']+' '+self.buttonrec['short_name']+' '+fieldvalue)
        self.lockstate = 1
        self._setlock()
